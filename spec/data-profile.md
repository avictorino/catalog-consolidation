...
it has already produced. `CatalogRepository.classify_source` inspects the table
...
Revision `0001` (`migrations/versions/0001_refactor_catalog.py`) holds the step
functions directly. Staged tables are built alongside the originals then swapped in. FK enforcement remains off during the rebuild because SQLite's
