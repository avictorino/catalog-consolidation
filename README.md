...
| DB access | SQLAlchemy Core (no ORM) + one Alembic revision, behind the `CatalogRepository` adapter in `consolidation.sqlite_store` | declarative schema + parameterized statements; the store owns the connection, drives the setup-transaction refactor, and enables/verifies foreign keys before JSON import |
