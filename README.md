...
| DB access | SQLAlchemy Core (no ORM) + one Alembic revision, behind a single `CatalogRepository` | declarative schema + parameterized statements; the repository owns the connection, drives the setup-transaction refactor, and enables/verifies foreign keys before JSON import |
