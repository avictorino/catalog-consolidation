...
from consolidation.resolver import CatalogIndex, CatalogProduct, download_to, resolve_product
from consolidation.similarity import DifflibSimilarity, RapidFuzzSimilarity, Similarity

from .conftest import PRODUCT_ROWS

_DL_URL = "https://example.com/catalog.db"


@responses.activate
def test_download_writes_temp_file(tmp_path: Path) -> None:
    body = b"SQLite format 3\x00" + b"\x00" * 512
    responses.add(responses.GET, _DL_URL, body=body, status=200)
    path = download_to(_DL_URL, tmp_path)
...
def test_download_http_error_leaves_no_temp(tmp_path: Path) -> None:
    responses.add(responses.GET, _DL_URL, status=404)
    with pytest.raises(requests.HTTPError):
        download_to(_DL_URL, tmp_path)
    assert list(tmp_path.glob("*.tmp")) == []


@responses.activate
def test_download_partial_failure_leaves_no_temp(tmp_path: Path) -> None:
    responses.add(responses.GET, _DL_URL, body=requests.ConnectionError("boom"))
