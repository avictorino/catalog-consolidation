"""The one shared string-normalization function.

Applied identically to catalog names, feed names, brand values, category values, and
the title-cased form persisted in ``Brand``/``Category``/``Seller``.``Name``. See
``spec/contract.md`` section 3. SQLite ``lower()`` is ASCII-only and cannot fold
accents, so this is done in Python.
"""
